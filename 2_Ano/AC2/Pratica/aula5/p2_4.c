...
void blink(unsigned char value, int blinkPeriod, int duration);
...

{
    ...
    while (1)
    {
        ...
        if (PORTBbits.RB0 == 1)
        {
            ...
            {
                displayCounter++;
                if (displayCounter == 59)
                {
                    blink(displayCounter, 200, 5000);
                    displayCounter = 0;
                }
            }
        }
...
void blink(unsigned char value, int blinkPeriod, int duration)
{
    ...
    int i;
    for (i = 0; i < duration; i += blinkPeriod * 2)
    {
        int counter;
        for (counter = 0; counter < blinkPeriod; counter += 10)
        {
            delay(10);
            send2displays(value); // ligar
        }
        for (counter = 0; counter < blinkPeriod; counter += 10)
        {
            delay(10);
