...
main:
    ...
    lw      $t3,PORTB($t1)      
    andi    $t3,$t3,0x0001      # read input rb3

    beq     $t3,0x0001,else         #1 0 0 0
        # deslocamente a direita -> desligado
        andi    $t4,$t0,0x0001      #0 0 0 1
        srl     $t0,$t0,1           #0 1 0 0
        bne     $t4,1,while
        ori     $t0,$t0,0x0008      #0 0 0 0
        j       while
    else:
        # deslocamento a esquerda -> ligado
        andi    $t4,$t0,0x0008      #0 0 0 1
        sll     $t0,$t0,1           #0 1 0 0
        bne     $t4,8,while
        ori     $t0,$t0,0x0001      #0 0 0 0
