...
int producer(uint32_t id, uint32_t niter)
{
    ...
    for (i = 0; i < niter; i++)
    {
        ...
        char msg[] = "Temos entrada";
        fifostrings::in(id, value, msg);

        /* do something else */
        gaussianDelay(10, 5);

        /* print them */
            printf("\e[30;00mThe value %05u with msg %s was produced by process P%02u!\e[0m\n", value, msg, id);
    }
...
int consumer(uint32_t id, uint32_t niter)
{
    ...
    for (i = 0; i < niter; i++)
    {
        ...
        uint32_t pid, value;
        char * msg;
        fifostrings::out(&pid, &value, &msg);

        /* print them */
        if (value == 99999 || pid == 99 || (value % 100) != pid)
            printf("\e[31;01mThe value %05u with msg %s was produced by process P%02u!\e[0m\n",
                    value, msg, pid);
        else
            printf("\e[32;01mThe value %05u with msg %s was produced by process P%02u!\e[0m\n",
                    value, msg, pid);
    }
